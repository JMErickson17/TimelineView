...
class MyViewController : UIViewController {
    override func loadView() {
        ...
        // Instantiate and configure the timeline
        let timeline = TimelineView()
        timeline.frame = CGRect(x: 100, y: 20, width: 200, height: 300)
        timeline.backgroundColor = .gray
        timeline.contentView.backgroundColor = .green
        timeline.topLabelText = "Top Label"
        timeline.bottomLabelText = "Bottom Label"
        
        // Add the timeline as a subview
        view.addSubview(timeline)
        
        // Use the timelines contentView to add custom subviews
        
        let contentLabel = UILabel()
        contentLabel.numberOfLines = 2
        contentLabel.text = "Your content\ngoes here"
        contentLabel.sizeToFit()
        timeline.contentView.addSubview(contentLabel)
